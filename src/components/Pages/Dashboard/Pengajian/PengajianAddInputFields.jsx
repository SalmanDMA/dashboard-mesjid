...
import { addPengajian } from '../../../../store/slices/pengajianSlices';

const PengajianAddInputFields = () => {
 ...
 const onSubmit = async (data) => {
  setIsLoading(true);
  try {
   await dispatch(addPengajian(data));
   setIsLoading(false);
   dispatch(successToastPengajian('Pengajian added successfully'));
   reset();
   navigate(linkPengajian);
  } catch (error) {
   setIsLoading(false);
   dispatch(failToastPengajian('Failed to add pengajian'));
   reset();
  }
 };
   ...
   <div className='drawer-content flex flex-col'>
    ...
    <div className='sm:px-14 pt-36 pb-14 w-full h-full bg-[#dddddd]'>
     <div className='container p-6 mx-auto bg-white rounded-lg shadow-lg'>
      <h2 className='text-2xl font-bold'>Create Pengajian</h2>
      <form className='mt-4 space-y-4' onSubmit={handleSubmit(onSubmit)}>
       ...
       <button type='submit' className='btn bg-primary_dark hover:bg-primary text-heading-5 px-4 py-2 text-white transition-all duration-300 ease-in-out' disabled={isLoading}>
        {isLoading ? 'Loading...' : 'Add Pengajian'}
       </button>
      </form>
     </div>
    </div>
...
export default PengajianAddInputFields;
